/**
 * AITuber相互認証・交流プロトコル 暗号ユーティリティ (Client)
 ...
 * オブジェクトを正準化する
 * プロパティをソートして再帰的に処理
 * パフォーマンス最適化: 事前キャパシティ確保
 */
function _canonicalizeObject(obj: Record<string, unknown>): string {
  const keys = Object.keys(obj);
  const len = keys.length;

  // ソート済みインデックス配列を作成（メモリ効率化）
  const sortedIndices = new Array<number>(len);
  ...
  sortedIndices.sort((a, b) => {
    ...
    for (let i = 0; i < minLen; i++) {
      const codeA = keyA.codePointAt(i) as number;
      const codeB = keyB.codePointAt(i) as number;
  ...
  for (let i = 0; i < len; i++) {
    const key = keys[sortedIndices[i]];
    const value = obj[key];
    // undefined は出力しない (JCS仕様)
    if (value !== undefined) {
      if (addedCount > 0) parts.push(',');
...
/**
 * Session用の鍵ペアを生成する
 * @param keyId 鍵ID (省略時は自動生成)
 * @returns 鍵ペア情報
 */
export async function generateSessionKeyPair(keyId?: string): Promise<{
  keyId: string;
  publicKey: string;
  privateKey: string;
}> {
  const keyPair = await generateKeyPair();
  const generatedKeyId = keyId || `session_${Date.now()}_${Math.random().toString(36).slice(2, 10)}`;
...
/**
 * 鍵ペアを安全にクリアする (メモリ上の機密データを消去)
 * 注: JavaScriptでは真のメモリクリアは保証されないため、
 * 本番環境ではより安全なストレージの使用を推奨
 */
